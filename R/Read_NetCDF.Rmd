...
>>>>>>> a0ac6bcdeb9575f8964fcdd60ebdc950a8a95fa6
```
...
# Use if you have access to Raw Curated data files
...
#open netCDF files
ncid_R<-nc_open(ncfname_raw) # Use if you have access to Raw Curated data files
ncid_P<-nc_open(ncfname_processed)
```
...
```{r plot tracking data}
#Processed files have compiled and interpolated track data which are represented at equal temporal distributions. 
  #Raw curated track data are not processed and so can over-represent presence data.
...
First, set up the `viridis` color scale to work with the Plotly interactive plotting environment
...
# 
# dive_R<-data.frame(diveDate_R,Depth_R)  #create a dataframe for plotting
# 
# #Plot raw curated dive record against time 
# TDR1_Ddat_R <- plot_ly(data = dive_R, x = ~diveDate_R, y = ~Depth_R, 
#                        type = 'scatter', 
#                        mode = 'lines+markers',
#                      #so that hover labels show full date-time data
#                      text = ~paste("Date-Time: ", diveDate_R, 
#                                    "<br>Depth: ", Depth_R), 
#                      hoverinfo = 'text') %>%
#   layout(
...
TDR1_Ddat_P <- plot_ly(data = dive_P, x = ~diveDateTime, y = ~diveDepth, 
=======
...
<<<<<<< HEAD
TDR1_Ddat_Pbar <- plot_ly(data = dive_P, x = ~diveDateTime, y = ~diveDepth, 
                       type = 'bar', 
                     #setting so that hover labels show full date-time data
                     text = ~paste("Date-Time: ", diveDateTime, 
                                   "<br>Depth: ", diveDepth, "m"), 
...
You can read in multiple nc files and plot their track and dive records to compare differences and similarities among records.
...
<<<<<<< HEAD
  ...
  if (metadata$Data_Track_QCFlag > 3) {
     message("FYI Track data have more than 3 QC flags. Data are not extracted.")
  } else {
    track_data <- tibble(
...
=======
  ...
  # Read track data if QC flag is appropriate
  if (metadata$Data_Track_QCFlag <= 3) {
    track_data <- tibble(
>>>>>>> a0ac6bcdeb9575f8964fcdd60ebdc950a8a95fa6
...
<<<<<<< HEAD
  ...
  if (metadata$Data_TDR1_QCFlag > 3) {
     message("FYI Dive data have more than 3 QC flags. Data are not extracted.")
  } else {
    dive_data <- tibble(
#Repeat the QC Flag, Deployment and Seal IDs for the length (Dsize) of the dive file
=======
  # Read dive data if QC flag is appropriate
  if (metadata$Data_TDR1_QCFlag <= 3) {
    dive_data <- tibble(
>>>>>>> a0ac6bcdeb9575f8964fcdd60ebdc950a8a95fa6
...
## Load ZOC'd full resolution (Raw Curated) TDR data
